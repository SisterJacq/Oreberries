package mortvana.oreberries.item.block;
...
public class OreberryBush2Item extends ItemBlock
{
    public int blockID;

    public OreberryBush2Item(int id)
    {
        super(id);
        blockID = id + 256;
        setHasSubtypes(true);
    ...
    public boolean onItemUse (ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ)
    {
        ...
        else if (player.canPlayerEdit(x, y, z, side, stack) && player.canPlayerEdit(x, y + 1, z, side, stack))
        {
            ...
            if (block != null && block.canSustainPlant(world, x, y, z, ForgeDirection.UP, (IPlantable) OreberryContent.oreberry2) && world.isAirBlock(x, y + 1, z))
            {
