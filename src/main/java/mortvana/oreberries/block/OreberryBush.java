...
import mortvana.oreberries.client.OreberryRender;
import mortvana.oreberries.common.OreberryHelper;
...
public class OreberryBush extends BlockLeavesBase implements IPlantable
{
    ...
    public void registerIcons (IconRegister iconRegister)
    {
        ...
        for (int i = 0; i < this.fastIcons.length; i++)
        {
            if (textureNames[i] != "")
            {
                this.fastIcons[i] = iconRegister.registerIcon("oreberries:bushes/" + textureNames[i] + "_fast");
                this.fancyIcons[i] = iconRegister.registerIcon("oreberries:bushes/" + textureNames[i] + "_fancy");
            }
        }
    }
    ...
    public void onBlockClicked (World world, int x, int y, int z, EntityPlayer player)
    {
        if (!world.isRemote)
        {
            ...
            if (meta >= 12)
            {
                ...
                OreberryHelper.spawnItemAtPlayer(player, new ItemStack(OreberryContent.oreberries, 1, meta % 4 + itemMeat));
            }
        }
    }
    ...
    public boolean onBlockActivated (World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9)
    {
        ...
        if (meta >= 12)
        {
            ...
            world.setBlock(x, y, z, blockID, meta - 4, 3);
            OreberryHelper.spawnItemAtPlayer(player, new ItemStack(OreberryContent.oreberries, random.nextInt(3) + 1, meta % 4 + itemMeat));
            return true;
        }
